...
/**
 * ============================================================================
 *  MetaNFTAuctionTransparent 部署模块（透明代理模式）
 * ============================================================================
 ...
 *   4. MetaNFTAuctionTransparent  透明代理「实现」合约（V1）
 *   5. TransparentUpgradeableProxy 代理合约（OZ v5.6 构造：(logic, initialOwner, data)，
 *      代理内部自动创建并持有 ProxyAdmin，admin = initialOwner = 部署账户）
 *   6. setTokenOracle ×2          自动把 ETH 与 USDC 的预言机注册进拍卖合约（onlyOwner）
 *
 *  升级方式：通过代理内部的 ProxyAdmin 执行 upgradeAndCall(proxy, newImpl, data)，
 *           仅 initialOwner（部署账户）有权限。
 *
 *  用法：
 *    npx hardhat ignition deploy ignition/modules/MetaNFTAuctionTransparent.ts \
...
export default buildModule("MetaNFTAuctionTransparent", (m) => {
  ...
  // 管理员（owner）：部署账户，也是代理内部 ProxyAdmin 的持有者
  ...
  // ==================== 3. 部署透明代理拍卖合约（实现 + 代理） ====================
  // 3.1 部署实现合约（V1，构造时已 _disableInitializers）
  const auctionImpl = m.contract("MetaNFTAuctionTransparent", [], {
    id: "AuctionImpl",
    after: [usdcOracle],
  });
  ...
  // 3.3 部署透明代理：(logic, initialOwner, data)
  //     —— 第二个参数是 initialOwner（部署账户），代理会自动创建 ProxyAdmin 并以此账户为 owner
  const proxy = m.contract("TransparentUpgradeableProxy", [auctionImpl, owner, initData], {
    id: "AuctionProxy",
  });

  // 3.4 用实现合约的 ABI 绑定代理地址，得到可直接交互的拍卖合约实例
  const auction = m.contractAt("MetaNFTAuctionTransparent", proxy, { id: "Auction" });
